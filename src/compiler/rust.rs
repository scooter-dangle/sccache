            ...
            let dir_entries = match fs::read_dir(crate_link_path) {
                ...
                Err(e) => {
                    return Err(Error::with_chain(
                        e,
                        "Failed to read dir entries in crate link path",
                    ))
                }
            };
            for entry in dir_entries {
                let entry = match entry {
                    ...
                    Err(e) => {
                        return Err(Error::with_chain(
                            e,
                            "Error during iteration over crate link path",
                        ))
                    }
                };
                let path = entry.path();
            ...
            for (_dep_info_dist_path, dep_info_local_path) in output_paths {
                ...
                if dep_info == dep_info_local_path {
                    ...
                    for (local_path, dist_path) in get_path_mappings(path_transformer) {
                        ...
                        deps = re
                            .replace_all(&deps, local_path.to_str().unwrap())
                            .into_owned();
                    }
...
impl RlibDepReader {
    fn new_with_check(executable: PathBuf, env_vars: &[(OsString, OsString)]) -> Result<Self> {
        ...
        let temp_dir =
            tempfile::tempdir().chain_err(|| "Could not create temporary file for rlib output")?;
