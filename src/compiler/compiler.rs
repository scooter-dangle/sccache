            ...
            Box::new(cache_status.then(move |result| {
                ...
                Box::new(compile.and_then(move |(cacheable, compiler_result)| {
                    ...
                    if !compiler_result.status.success() {
                        ...
                        return f_ok((CompileResult::CompileFailed, compiler_result)) as SFuture<_>;
                    }
                            ...
                            .and_then(move |mut entry| {
                                ...
                                let future = storage.put(&key, entry).then(move |res| {
                                    match res {
                                        ...
                                        Err(ref e) => {
                                            debug!("[{}]: Cache write error: {:?}", out_pretty, e)
                                        }
                                    }
    ...
    T: CommandCreatorSync,
{
    ...
    let (compile_cmd, _dist_compile_cmd, cacheable) = compilation
        ...
        .unwrap();
    Box::new(compile_cmd.execute(&creator).map(move |o| (cacheable, o)))
}
    ...
    Box::new(
        ...
        .and_then(move |dist_compile_cmd| {
            ...
            let dist_output_paths = compilation
                ...
                .unwrap();
            compilation
                .into_dist_packagers(path_transformer)
                .map(|packagers| (dist_compile_cmd, packagers, dist_output_paths))
        })
        .and_then(
            move |(
                mut dist_compile_cmd,
                (inputs_packager, toolchain_packager, outputs_rewriter),
                dist_output_paths,
            )| {
                debug!(
                ...
                let (dist_toolchain, maybe_dist_compile_executable) =
                    ftry!(dist_client.put_toolchain(
                        &local_executable,
                        &weak_toolchain_key,
                        toolchain_packager
                    ));
                    ...
                    dist_client
                        ...
                        .and_then(move |jares| {
                            let alloc = match jares {
                                ...
                                dist::AllocJobResult::Fail { msg } => f_err(Error::with_chain(
                                    Error::from("Failed to allocate job"),
                                    msg,
                                )),
                            };
                            alloc.and_then(move |job_alloc| {
                        ...
                        .map(move |(jres, path_transformer)| {
                            ...
                            for (path, output_data) in jc.outputs {
                                ...
                                let count =
                                    io::copy(&mut output_data.into_reader(), &mut file).unwrap();
                                assert!(count == len);
                                output_paths.push((path, local_path))
                            }
                            outputs_rewriter
                                .handle_outputs(&path_transformer, output_paths)
                                .unwrap();
                            jc.output.into()
                        }),
                )
...
impl OutputsRewriter for NoopOutputsRewriter {
    fn handle_outputs(
        self: Box<Self>,
        _path_transformer: &dist::PathTransformer,
        _output_paths: Vec<(String, PathBuf)>,
    ) -> Result<()> {
        Ok(())
    }
}
...
impl fmt::Debug for CompileResult {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &CompileResult::CacheMiss(ref m, ref d, _) => {
                write!(f, "CompileResult::CacheMiss({:?}, {:?}, _)", d, m)
            }
...
impl PartialEq<CompileResult> for CompileResult {
    fn eq(&self, other: &CompileResult) -> bool {
        match (self, other) {
            ...
            (&CompileResult::CacheMiss(ref m, _, _), &CompileResult::CacheMiss(ref n, _, _)) => {
                m == n
            }
