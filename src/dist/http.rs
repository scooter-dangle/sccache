...
mod common {
    ...
    impl ReqwestRequestBuilderExt for reqwest::RequestBuilder {
        ...
        fn bearer_auth(&mut self, token: String) -> &mut Self {
            self.header(reqwest::header::Authorization(reqwest::header::Bearer {
                token,
            }))
        }
    }
    ...
    pub fn bincode_req_fut<T: serde::de::DeserializeOwned + 'static>(
        req: &mut reqwest::unstable::async::RequestBuilder,
    ) -> SFuture<T> {
        Box::new(
            req.send()
                .map_err(Into::into)
    ...
    impl Client {
        pub fn new(
            ...
            let client = reqwest::ClientBuilder::new()
                ...
                .build()
                .unwrap();
            let client_async = reqwest::unstable::async::ClientBuilder::new()
                .timeout(timeout)
                .build(handle)
                .unwrap();
            Self {
