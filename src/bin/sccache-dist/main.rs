...
use sccache::dist::{
    ...
    InputsReader, JobAlloc, JobComplete, JobId, JobState, RunJobResult, SchedulerIncoming,
    SchedulerOutgoing, ServerId, ServerIncoming, ServerOutgoing, StatusResult,
    SubmitToolchainResult, TcCache, Toolchain, ToolchainReader, UpdateJobStateResult,
};
...
fn run(command: Command) -> Result<i32> {
    match command {
        ...
        Command::Auth(AuthSubcommand::JwtHS256ServerToken {
            ...
            println!(
                "{}",
                create_jwt_server_token(server_id, &header, &secret_key)
            );
            Ok(0)
        }
        ...
        Command::Scheduler(scheduler_config::Config {
            ...
            let check_client_auth: Box<Fn(&str) -> bool + Send + Sync> = match client_auth {
                scheduler_config::ClientAuth::Insecure => {
                    Box::new(move |s| s == INSECURE_DIST_CLIENT_TOKEN)
                }
                scheduler_config::ClientAuth::Token { token } => Box::new(move |s| s == token),
            };

            let check_server_auth: Box<Fn(&str) -> Option<ServerId> + Send + Sync> =
                match server_auth {
                    scheduler_config::ServerAuth::Insecure => {
                    ...
                    scheduler_config::ServerAuth::JwtHS256 { secret_key } => {
                        let secret_key =
                            base64::decode_config(&secret_key, base64::URL_SAFE_NO_PAD)
            ...
            let http_scheduler =
                dist::http::Scheduler::new(scheduler, check_client_auth, check_server_auth);
    ...
    fn handle_alloc_job(
        ...
        let (job_id, server_id, auth) = {
            ...
            if let Some((server_id, server_details)) = best {
                ...
                info!(
                    "Job {} created and assigned to server {:?}",
                    job_id, server_id
                );
                assert!(
                    jobs.insert(
        ...
        let AssignJobResult { need_toolchain } = requester
